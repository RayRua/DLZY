...
void DRV750_Motor_Config(uint8_t addr){
			...
			delay_ms(4);
			DRV750_POSITION_Config(addr,P_RUN_MODE_INCREMENTAL_DRV750_POSITION_MODE,50,50,10,0);//DRV750_VELOCITY initial
			delay_ms(4);
...
void DRV750_POSITION_Control(uint8_t addr,float target){
			...
			if(target>0)dir=CW;
			else {dir=CCW;target=-target;}
			DRV750_Postion_Control_Set_Per_Target(addr,(uint32_t)(target*GIMBAL_DEGREE_TO_PULSE_NUM_RATIO));
			DRV750_DRV750_POSITION_Enable_Mode mode;
			if(dir==CW)mode=P_ENABLE_FORWARD_ONCE;
			else mode=P_ENABLE_BACKWARD_ONCE;
			osDelay(4);
...
void DRV750_VELOCITY_Control(uint8_t addr, int32_t rpm){
			...
			if(rpm>9000000)rpm=9000000;
			else if(rpm<-9000000)rpm=-9000000;
...
//			osDelay(4);
//			
//			RS485_Send(D_AI_Read_Parameter(addr,H0B_71,2),FIX_LENGTH);
//			
//			osDelay(4);
//			pitch_current_status.gimbal_pitch_absolute_echo=re.data[0]|(re.data[1]<<16);
//}
