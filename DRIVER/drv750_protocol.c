...
static uint16_t CRC16_Check_Sum(volatile uint8_t* pchMessage, uint32_t dwLength)
{
...
static Response response_data;
//had tested
uint8_t* DRV750_Request_Read_Register(uint8_t addr,uint16_t register_start_addr,uint16_t register_num){
		
			request[0]=  addr;
			request[1]=  CMD_READ_REGISTER;					//0x03
			request[2]= (register_start_addr>>8)&0xFF;
			request[3]=  register_start_addr&0xFF;
			request[4]= (register_num>>8)&0xFF;
			request[5]=  register_num&0xFF;
		uint16_t crc = Modbus_CRC16_List(request, 6);
			request[6]=  crc&0xFF;
...
uint8_t* DRV750_Request_Write_Single_Register(uint8_t addr,uint16_t single_register_addr,uint16_t data){
			...
			request[0]=  addr;
			request[1]=  CMD_WRITE_SINGLE_DATA;			//0x06
			request[2]= (single_register_addr>>8)&0xFF;
			request[3]=  single_register_addr&0xFF;
...
uint8_t* DRV750_Request_Write_Mul_Register(uint8_t addr,uint16_t register_start_addr,uint16_t register_num,uint16_t* data,uint8_t* total_num){
		...
		*total_num=1+1+2+2+1+data_length;//without crc
			request[0]=  addr;
			request[1]=  CMD_WRITE_MULTIPLE_DATA;		//0x10
			request[2]= (register_start_addr>>8)&0xFF;
			request[3]=  register_start_addr&0xFF;
			request[4]= (register_num>>8)&0xFF;
			request[5]=  register_num&0xFF;
		...
		uint8_t i=0;
			for(;i<register_num;i++){
				...
				request[7+i*2+1]=  data[i]&0xFF;
			}
		uint16_t crc = Modbus_CRC16_List(request, *total_num);
			request[7+i*2]=  crc&0xFF;
			request[7+i*2+1]=  (crc>>8)&0xFF;
			*total_num+=2;
			return request;
}
/*****************************DRV750_REQUEST************************************/
...
_Bool DRV750_Response_Register(volatile uint8_t* response){
		...
		if(response!=NULL&&response[1]==0x03){
			if(CRC16_Check_Sum(response,1+1+1+response[2]+2)){
			response_data.addr=response[0];
			response_data.cmd=response[1];
			response_data.data_length=response[2];
			response_data.register_num=response[2]/2;
			for(uint8_t i=0;i<response_data.register_num;i++){
				response_data.data[i]=((uint16_t)response[3+i*2]<<8)|response[3+i*2+1];
			}
		}
			else return 0;
	}
		else if(response!=NULL&&response[1]&0x80){
			if(CRC16_Check_Sum(response,5)){
			response_data.addr=response[0];
			response_data.cmd=response[1];
			response_data.error_code=response[2];
			}
			else return 0;
		}
