...
int32_t WitSerialWriteRegister(SerialWrite Write_func)//give the function point value
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_MODBUS:
            if(s_uiWitDataCnt > 2)
            {
                ...
                usTemp = s_ucWitDataBuff[2] >> 1;//usTemp num of reg
                for(i = 0; i < usTemp; i++)
                {
                    sReg[i+s_uiReadRegIndex] = ((uint16_t)s_ucWitDataBuff[(i<<1)+3] << 8) | s_ucWitDataBuff[(i<<1)+4];// Prevent data interference with each other
                }
                p_WitRegUpdateCbFunc(s_uiReadRegIndex, usTemp);//ustemp num of reg
                s_uiWitDataCnt = 0;
								cmd_lost_imu_time=0;
            }
        break;
...
int32_t WitWriteReg(uint32_t uiReg, uint16_t usData)//uireg address usData num length and write single reg
{
    uint16_t usCRC;
    uint8_t ucBuff[8];
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_MODBUS:
            ...
            ucBuff[6] = usCRC >> 8;
            ucBuff[7] = usCRC & 0xff;
            p_WitSerialWriteFunc(ucBuff, 8); //send
            break;
        case WIT_PROTOCOL_CAN:
...
int32_t WitRedRoll_Pitch(void)
{	
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(5);
	else if(s_uiProtoclo == WIT_PROTOCOL_NORMAL) p_WitDelaymsFunc(1);
	else ;
	if(WitReadReg(Roll, 0x0002) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(5);
	return WIT_HAL_OK;
}

int32_t WitRedStart(void)
{	
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(5);
	else if(s_uiProtoclo == WIT_PROTOCOL_NORMAL) p_WitDelaymsFunc(1);
	else ;
	if(WitWriteReg(SAVE, 0x00FF) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(5);
	return WIT_HAL_OK;
}
