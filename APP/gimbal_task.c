...
/**********************remote_control**************************/
	...
	if(flag_remote_shut|(cmd_lost_flag&&MBUS_ON_OFF==0x10)) {	
		remote.right_key=0;
		remote.left_key=0;
		...
		if(count_gimbal>10){LED1 = !LED1;}//if the motor switching mode is abnormal, LED1 will not blink
		if(count_gimbal>10)count_gimbal=0;
		return ;
	}
	...
	PITCH_Read_Status();
		...
		if(count_gimbal>10)count_gimbal=0;//if the motor switching mode is abnormal, LED1 will not blink
}
		if(count_gimbal>10){LED1 = !LED1;count_gimbal=10;}
		
	servo_old_torque(SERVO_LETF,SERVO_TORQUE_ON);
	servo_old_torque(SERVO_RIGHT,SERVO_TORQUE_ON);


	if(MBUS_ON_OFF==0x00){ //remote_control
	//while
	remote_process();
	...
	if((gimbal.yaw>0.1f||gimbal.yaw<-0.1f)&&(yaw_current_status.mode!=DRV750_VELOCITY)){
		DRV750_Motor_Control_SHIFIT(GIMBAL_YAW,DRV750_VELOCITY);yaw_target_status.mode=DRV750_VELOCITY;
	}
	if((gimbal.pitch>0.1f||gimbal.pitch<-0.1f)&&(pitch_current_status.mode!=D_AI_VELOCITY)){
		D_AI_Motor_Control_SHIFIT(GIMBAL_PITCH,D_AI_VELOCITY);	pitch_target_status.mode=D_AI_VELOCITY;
	}
//	if(gimbal.pitch==0)
//		pitch_remotecontrol_stop();
//	if(gimbal.yaw==0)
...
/**********************remote_control**************************/
/**********************master_control**************************/
	if(master_receive_flag&&MBUS_ON_OFF==0x10){//master 
		/*servo*/
		while(!gimbal_master.left_shoot){
			if(last_temp_left==gimbal_master.left_shoot)break;
		...
		while(!gimbal_master.right_shoot){
			if(last_temp_right==gimbal_master.right_shoot)break;
		...
		if(gimbal_master.master_status==MASTER_POSITION){
			...
			if(yaw_current_status.mode!=DRV750_POSITION){
				...
				if(pd4_level==0 && gimbal.pitch>0){D_AI_POSITION_Stop(GIMBAL_PITCH);gimbal.pitch=0;}//limit position
				if(pd5_level==0 && gimbal.pitch<0){D_AI_POSITION_Stop(GIMBAL_PITCH);gimbal.pitch=0;}
		
				if(pd4_level==0 && gimbal.pitch<0){D_AI_POSITION_Start(GIMBAL_PITCH);gimbal.pitch=0;}
				if(pd5_level==0 && gimbal.pitch>0){D_AI_POSITION_Start(GIMBAL_PITCH);gimbal.pitch=0;}
			}
		...
		/*velocity*/
			...
			if(yaw_current_status.mode!=yaw_target_status.mode)	DRV750_Motor_Control_SHIFIT(GIMBAL_YAW,DRV750_VELOCITY);
			if(pitch_current_status.mode!=pitch_target_status.mode)D_AI_Motor_Control_SHIFIT(GIMBAL_PITCH,D_AI_VELOCITY);
			motor_velocity_control();
		}
...
void remote_process(){//Different remote controls need to be adapted according to the specific channel values
		...
		switch(remote.left_key){  //direct take maybe have wrong data
					case  200:temp_key5=1;break;
					case 1000:temp_key5=0;break;
					case 1800:temp_key5=2;break;
				}
		switch(remote.right_key){
					case  200:temp_key6=1;break;
					case 1000:temp_key6=0;break;
					case 1800:temp_key6=2;break;
				}
		...
		if(remote.channel4>100){
				...
				gimbal.yaw=(int32_t)remote.channel4/398.0f*GIMBAL_YAW_MAX_RPM;
		}
		else if(remote.channel4<-100){
				
				gimbal.yaw=(int32_t)remote.channel4/400.0*GIMBAL_YAW_MAX_RPM;
		}
		else
				gimbal.yaw=0;
//		//pitch
//		if(remote.channel3>220){
//			
//				gimbal.pitch=(int32_t)remote.channel3/756.0*GIMBAL_PITCH_MAX_RPM;
//		}
//		else if(remote.channel3<-220){
//				
//				gimbal.pitch=(int32_t)remote.channel3/832.0*GIMBAL_PITCH_MAX_RPM;
//		}
//		else
//				gimbal.pitch=0;
//}
...
//		servo_old_target(SERVO_LETF,2048,5);
	...
	//pitch
	if(gimbal.pitch_position){//substitute pitch as shoot
		if(pitch_current_status.mode!=D_AI_POSITION)
			D_AI_Motor_Control_SHIFIT(GIMBAL_PITCH,D_AI_POSITION);
...
void motor_velocity_control(){
	...
	if(pd4_level==0 && gimbal.pitch>0) gimbal.pitch=0;//limit velocity
	if(pd5_level==0 && gimbal.pitch<0) gimbal.pitch=0;
...
void yaw_remotecontrol_stop(void){
			gimbal.yaw=0;
			DRV750_VELOCITY_Control(GIMBAL_PITCH, gimbal.yaw);
}
